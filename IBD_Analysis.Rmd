...
Although original logistic model throws a warning, the model does predict the correct outcome for 86% of the observations. 
...
step_inf2 <- glm(formula = data_inf$`data2$inf_comp` ~ gender + cpt.code + 
    elective.surgery + height + weight + dyspnea + history.of.severe.copd + 
    steroid.immunosuppressant.use.for.chronic.condition + serum.sodium + 
    serum.creatinine + hematocrit + emergency.case + X..of.other.procedures + 
    ...
    immune.modulation.60.days.prior.to.surgery, family = "binomial", 
    data = data_inf)
...
step_any2 <- glm(formula = data_any$`data2$any_comp` ~ cpt.code + elective.surgery + 
    origin.status + functional.heath.status + open.wound.with.or.without.infection + 
    steroid.immunosuppressant.use.for.chronic.condition + serum.sodium + 
    serum.creatinine + emergency.case + X..of.concurrent.procedures + 
    duration.of.surgical.procedure..in.minutes. + colectomy.chemotherapy.w.in.90.days + 
    ileostomy.formation, family = "binomial", data = data_any)
